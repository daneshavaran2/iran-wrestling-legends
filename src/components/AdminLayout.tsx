...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex">
        ...
        <GlassCard className="h-full rounded-none border-l border-border/50">
          <div className="flex flex-col h-full p-4">
            <div className="flex items-center justify-between mb-8">
              {sidebarOpen && (
                <h1 className="text-xl font-bold text-gold animate-fade-in">پنل مدیریت</h1>
              )}
              <button
            ...
            <div className="space-y-2 pt-4 border-t border-border/50">
              ...
              >
                ...
                {sidebarOpen && <span>{theme === 'dark' ? 'حالت روز' : 'حالت شب'}</span>}
              </button>
              ...
              >
                ...
                {sidebarOpen && <span>بازگشت به موزه</span>}
              </Link>
            </div>
          </div>
        </GlassCard>
      ...
      <main className={cn('flex-1 transition-all duration-300', sidebarOpen ? 'mr-64' : 'mr-20')}>
